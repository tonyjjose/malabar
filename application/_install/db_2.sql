 ...
 *
 * version 0.2
 * Changed the bigint(20) to datetime for timestamps.
 * Included the drop table at the beginning.
 */
...
DROP TABLE `bcc_app`.`users`;
DROP TABLE `bcc_app`.`category`;
DROP TABLE `bcc_app`.`courses`;
DROP TABLE `bcc_app`.`student_course`;
DROP TABLE `bcc_app`.`activity`;  
/*-------------------------------------------*/
...
CREATE TABLE IF NOT EXISTS `bcc_app`.`users` (
 ...
 `user_creation_timestamp` datetime NOT NULL, -- use NOW() function of sql while INSERT and UPDATE
 `user_last_login_timestamp` datetime DEFAULT NULL,
  PRIMARY KEY (`user_id`),
