 ...
 * ManagerObject class
 *
 * This is the Manager object. It provides static methods for DB operations as well us helper methods
...
class Manager extends user
{
        ...
        $mobile, $address, $course_mode,$approved, $active, $anon, $created, $last_login)
    {
        ...
        $mobile, $address, $course_mode, ROLE_MANAGER, $approved, $active, $anon, $created, $last_login);
    }

    /**
     * Create all Manager object array
     * @return array[] of object Manager or null
     */
    public static function getAllManagers()
    {
        ...
        foreach ($rows as $row) {
            $managers[] = new Manager($row->user_id,$row->user_name,$row->user_password_hash,$row->user_email,$row->user_age,
    ...
    /**
     * Check for user type Manager.
     * @return bool status
     */
    public static function isUserManager($id)
    {
        return (User::getUserType($id) == ROLE_MANAGER);
    } 
}